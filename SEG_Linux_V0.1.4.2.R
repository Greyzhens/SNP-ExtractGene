...
#####
#文件路径
WD = ("/home/grayzhens/R/Env4")  ##此处需要更改
#GWAS结果文件
print("正在读取结果文件")
data = fread("/home/grayzhens/R/Env4/NG_Env4_MLM8.txt",#此处为结果文件，需使用绝对路径 ##此处需要更改
             header = F)

#gff文件
gff_gnm = c("/Volumes/Grayzhens/学术资料/宁东臻/资源库/PeanutBaseData/GFF3/gnm1.ann1.CCJH/gnm1.ann1.CCJH.gene_models_main.gff3",
            "/home/grayzhens/Data/arahy.Tifrunner.gnm2.ann1.4K0L.gene_models_main.gff3")

hapmap_path = "/home/grayzhens/Data/all.snp.1781835.hapmap.hmp.txt"
print("正在读取hapmap文件：")
print(hapmap_path)
hapmap = fread(hapmap_path, header = T)
...
GWAS_ExtractGenes = function(version){
  ...
  for (traitnum in 1:traitcount) {
    ...
    for (chrnum in 1:20) {#20
      ...
      chr = cbind(chr, alpha)
      #只提取(3,12)
      chr = subset(chr, alpha >= 3 & alpha <= 12)
      ...
      if (nrow(chr) != 0) {#有显著性位点则输出提示
        ...
        genetic_typing_out = data.frame(matrix(nrow = snpcount, ncol = 287))
        ...
        for (snpnum in 1:snpcount) {
          ...
          Progress = snpnum / snpcount
          Yann3 = paste(trait_name," Chr",chrnum," SNP ",chr_snp$Pos[snpnum]," process (",snpnum ,"/",snpcount , ")"," Progress ", sprintf("%.2f", Progress * 100),"%",sep = "")
          print(Yann3)
