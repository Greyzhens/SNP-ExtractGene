...
#####
#文件路径
WD = ("/Volumes/Grayzhens/学术资料/宁东臻/毕业论文/GWAS/MLM")  ##此处需要更改
#GWAS结果文件
data = fread("/Volumes/Grayzhens/学术资料/宁东臻/毕业论文/GWAS/MLM/NG_Env1_MLM8.txt",#此处为结果文件，需使用绝对路径 ##此处需要更改
             header = F)

#gff文件
#gff文件
gff_gnm = c("/Volumes/Grayzhens/学术资料/宁东臻/资源库/PeanutBaseData/GFF3/gnm1.ann1.CCJH/gnm1.ann1.CCJH.gene_models_main.gff3",
            "/Volumes/Grayzhens/学术资料/宁东臻/资源库/PeanutBaseData/GFF3/gnm2.ann1.4K0L/gnm2.ann1.4K0L.gene_models_main.gff3")


hapmap_path = "/Volumes/Grayzhens/学术资料/NG群体原始数据/all.snp.1781835.W272.hmp.txt"
print("正在读取hapmap文件：")
print(hapmap_path)
              ...
              if (chr_snp$Pos[snpnum] <= chr_gff$endpos[gffnum]) {
                ...
                for (e in 1:geneexoncount) {
                  if (chr_snp$Pos[snpnum] >= geneexon$startpos[exonnum] & chr_snp$Pos[snpnum] <= geneexon$endpos[exonnum]) {
                    grayzhens = paste0(chr_gff$GeneName[gffnum],"在内含子中")
                    print(grayzhens)
                    exon = 1
                  }else{
                    exon = 0
