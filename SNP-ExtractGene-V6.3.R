...
#####
#文件路径
WD = ("/Volumes/Cornucopia/研究项目/花生荚果籽仁耐低钙相关基因定位研究/GWAS/结果/[6级]品种等级与病情指数/GWAS结果/CMLM")  ##此处需要更改
#GWAS结果文件
data = fread("/Volumes/Cornucopia/研究项目/花生荚果籽仁耐低钙相关基因定位研究/GWAS/结果/[6级]品种等级与病情指数/GWAS结果/CMLM/CaCMLM9.txt",#此处为结果文件，需使用绝对路径 ##此处需要更改
             header = F)
data2 = data[which(data$V7!=0)]
data=data2

#gff文件
gff_gnm = c("/Volumes/Cornucopia/研究生/数据库/公共资源/PeanutBase/GFF3/gnm1.ann1.CCJH/gnm1.ann1.CCJH.gene_models_main.gff3",
            "/Volumes/Cornucopia/研究生/数据库/公共资源/PeanutBase/GFF3/gnm2.ann1.4K0L/gnm2.ann1.4K0L.gene_models_main.gff3")

hapmap = fread("/Volumes/Cornucopia/研究生/数据库/私有资源/关联分析群体/1781835/all.snp.1781835.W272.hmp.txt" , header = T)

#GWAS：显著性阈值确定——GEC
#常用的显著性阈值确定方法是：
#Bonferroni correction = 显著性水平（0.01/0.05）/检验次数（number of detected markers）
#计算上层阈值
snp_number = nrow(hapmap)
#y_threshold = 0.01/snp_number
y_threshold = 0.05/snp_number
